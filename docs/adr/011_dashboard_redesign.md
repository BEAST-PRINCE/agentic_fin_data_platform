...
As the Multi-Agent system grew to take multiple sequential model calls, a simple Streamlit spinner was no longer an acceptable user experience. Furthermore, because intermediate agents output strict JSON packages, the frontend needed to isolate that raw JSON from the final synthesized answer. The current React UI presents the completed workflow steps after the request returns; it does not show them in real time.
