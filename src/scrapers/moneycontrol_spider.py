...
async def scrape_moneycontrol(url: str):
    """
    Scrape a Moneycontrol page using Playwright.
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not content:
                ...
                selectors = ['div.articleBody', 'div.content_wrapper', 'article', 'div[class*="content"]']
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        url = sys.argv[1]
        result = asyncio.run(scrape_moneycontrol(url))
        print(json.dumps(result, indent=2))
    else:
        print("Please provide a Moneycontrol URL to scrape.")
